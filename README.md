...
- Rename the keyword `tora` in any foldername (/tora and /docs/disalgname), in any filenames (docs/tora/tora.rst) and in any fields in any file (all instances in index.rst) to a **meaningfull abbreviation** of your distributed algorithm. Note that doc/substitutions.rst defines a keywork |tora| that has to be changed as well.
...
- Populate the rst files under the docs/tora: astract, algorithm, conclusion, introduction, results with the extension rst following restructured text syntax. The templates will guide you on what to write in each file. There is also a rubric for self-assessment.
- List your modules in code.rst under docs/tora
